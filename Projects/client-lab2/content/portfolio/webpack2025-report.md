...
- добавлена зависимость `bootstrap` в `package.json`;
- настроен запуск приложения через команду `npm run dev`;
- подготовлена Bootstrap-разметка страницы с колонками в соотношении `2-8-2`;
- в центральной колонке размещена большая синяя кнопка `Показать время`;
...
- выполнен запуск приложения через `Vite` и проверка результата в браузере.
...
В ней размещена большая синяя кнопка:

```text
Показать время
```

первый этап у меня голубой, и кнопку во втором этапе я захотел сделать синей.

Кнопка занимает всю ширину центральной колонки благодаря классам Bootstrap:

```html
<button id="showTimeButton" type="button" class="btn btn-primary btn-lg w-100 py-3">
  Показать время
</button>
```
...
6. Настройка большой синей кнопки `Показать время`.
7. Реализация модального окна с именем исполнителя и временем.
8. Запуск приложения через `npm run dev`.
9. Проверка результата в браузере.
